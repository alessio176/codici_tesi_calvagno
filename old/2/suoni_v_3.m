function [y_dx,y_sx] = suoni_v_3(opt)
...
%suono, ma anche come segnale di origine (queste sinusoidi che partono non
%esattamente all' inizio del ciclo non si percepiscono dall' audio, ma non
%si sa mai...).
...
DC_sx = ((1/opt.freq_base_sx)*opt.N*opt.freq_rip_sx)*100; %<- il *100 ci vuole perchè matlab vuole il numero come percentuale
...
beep_sx = sin(2*pi*opt.freq_base_sx*t(1:round(opt.N*(1/opt.freq_base_sx)*fs)+1));
...
% for i = 1:length(differenza_sx)
%     idx_in_alto = find(differenza_sx(i:end)==1,1)+(i-1);
%     idx_in_basso = find(differenza_sx(i:end)==-1,2)+(i-1);
%     y_sx(idx_in_alto:idx_in_basso(2)) = beep_sx;
%    
%     if isequal(i,1577)
%         disp('debug');
%     end
